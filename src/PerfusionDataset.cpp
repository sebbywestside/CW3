...
PerfusionDataset::PerfusionDataset() 
    : arrivalFrame(-1), arrivalSignal(0.0f), 
      peakFrame(-1), peakSignal(0.0f), 
      lvbpMask(0, 0), temporalGradient(0.0f) {
...
void PerfusionDataset::plotSignalTimecourse() const {
    ...
    std::vector<float> xValues(signalTimecourse.size());
    ...
    float yMargin = (maxSignal - minSignal) * 0.1f;  // 10% margin
    
    debug::log("Signal range: " + std::to_string(minSignal) + " to " + std::to_string(maxSignal));
    
    // Plot signal timecourse
    TG::plot(512, 200)
        .set_xlim(0, signalTimecourse.size() - 1)
        .set_ylim(minSignal - yMargin, maxSignal + yMargin)
        .set_grid(2, 20)
        .add_line(xValues, signalTimecourse, 2);  // Yellow line
}
...
void PerfusionDataset::plotGradientTimecourse() const {
    ...
    // Create x-axis (frame numbers)
    std::vector<float> xValues(gradientTimecourse.size());
    for (size_t i = 0; i < xValues.size(); ++i) {
        xValues[i] = i;
    }
    ...
    float yMargin = (maxGradient - minGradient) * 0.1f;  // 10% margin
    
    debug::log("Gradient range: " + std::to_string(minGradient) + " to " + std::to_string(maxGradient));
    
    // Plot gradient timecourse
    TG::plot(512, 200)
        .set_xlim(0, gradientTimecourse.size() - 1)
        .set_ylim(minGradient - yMargin, maxGradient + yMargin)
        .set_grid(2, 10)
        .add_line(xValues, gradientTimecourse, 3);  // Magenta line
}
